...
- (void) setProductsList:(NSSet *) value {
	NSLog(@"set prods dziala");
	
	if (productsList == nil) {
		productsList = [[NSMutableSet alloc] init];
	}
	
	if ([value count] > 0) NSLog(@"cos tu jest");
...
{
	#ifndef NDEBUG
	NSLog(@"IAP requesting products data");
	#endif

	if ([productsList count] == 0) NSLog(@"mamy problem");
...
NSLog(@"trying request");
if ([self.purchasableObjects count] == 0) NSLog(@"zero produktow");
#ifndef NDEBUG	
		   ...
		   forReceipt:(NSData*) receiptData
{
	...
	NSRange range = [productIdentifier rangeOfString: bundleID];
	if (range.location == NSNotFound) NSLog(@"bundle id not found");

	NSString *countText = [productIdentifier substringFromIndex:range.location+[bundleID length]];
...
bool consumeProduct(String product) {
	...
	return [[MKStoreManager sharedManager] consumeProduct:kFeatureID];
}
